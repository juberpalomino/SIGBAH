...
//				$('#txt_nombre_secundario').val(obj.idAlmacen);
				...
				$('#hid_id_personal_externo').val(obj.personalExterno);
...
//			$('#div_imp_pdf').modal('hide');
	...
	$('#btn_gra_per_ext').click(function(e) {
		...
		if (bootstrapValidator.isValid()) {
			...
			var params = { 
				idPersonalExterno :	$('#hid_id_personal').val(),
				nombres : $('#txt_nom_per').val(),
				cargo : $('#txt_ape_per').val(),
				telefono : $('#txt_car_per').val(),
				flagActivo : '1',
				codUbigeo : codUbigeo,
				tipoPersonal : flag
			};
...
function inicializarDatos() {
	...
	$('#li_tablas_generales').attr('class', 'active');
	$('#li_tablas_generales').closest('li').children('a').attr('href', '#');
	limpiarTablas();
...
function mostrarTablaTipMovimiento(respuesta) {
	...
	tbl_tip_movimiento.dataTable({
		...
		columns : [ {
			...
			data : 'flagDonacionIngreso'
		}, {
			data : 'flagDonacionSalida'
		}, {
			data : 'flagActivo'
		} ],
		language : {
...
function cargarCorrelativo(codigo) {
	...
	consultarAjax('GET', '/administracion/tablas-generales/obtenerCodigoAlmacenExterno', params, function(respuesta) {
		if (respuesta.codigoRespuesta == NOTIFICACION_ERROR) {
			...
			$('#txt_codigo_ext').val(respuesta.codAlmacen);       	
		}
		loadding(false);
	});
