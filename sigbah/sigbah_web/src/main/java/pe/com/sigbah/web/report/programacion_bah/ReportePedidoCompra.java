...
public class ReportePedidoCompra implements Serializable {
	 ...
	 * @throws Exception 
	 */
	public HSSFWorkbook generaReporteExcelPedidoCompra(List<PedidoCompraBean> lista) throws Exception {
		HSSFWorkbook wb = new HSSFWorkbook();
		try {				
	        HSSFSheet sheet = wb.createSheet("REGISTRO DE PEDIDO DE COMPRA");
	        ...
	        row1.createCell(4).setCellValue("N° pedido de compra");
	        ...
	        row1.createCell(7).setCellValue("DDE");
	        ...
	        for (PedidoCompraBean pedido : lista) {
		        ...
		        rows.createCell(2).setCellValue(pedido.getCodAnio());
		        rows.getCell(2).setCellStyle(style_cell);
		        
		        rows.createCell(3).setCellValue(pedido.getNomMes());
		        rows.getCell(3).setCellStyle(style_cell);
		        
		        rows.createCell(4).setCellValue(pedido.getNumPedidoCompra());
		        rows.getCell(4).setCellStyle(style_cell);
		        
		        rows.createCell(5).setCellValue(pedido.getFecPedido());
